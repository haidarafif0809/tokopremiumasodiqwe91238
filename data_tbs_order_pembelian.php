...
$session_id = session_id();
  ...
  $pilih_akses_tombol = $db->query("SELECT edit_produk_order_pembelian, hapus_produk_order_pembelian FROM otoritas_form_order_pembelian WHERE id_otoritas = '$_SESSION[otoritas_id]' ");
...
$sql =" SELECT tp.id,tp.kode_barang,tp.satuan,tp.nama_barang,tp.jumlah_barang,tp.harga,tp.subtotal,tp.potongan,tp.tax,s.nama";
$sql.=" FROM tbs_pembelian_order tp INNER JOIN satuan s ON tp.satuan = s.id ";
...
if ($otoritas_tombol['hapus_produk_order_pembelian'] > 0) {

      $nestedData[] = "<button class='btn btn-danger btn-hapus btn-sm' id='btn-hapus-".$row['id']."' data-id='". $row['id'] ."' data-kode-barang='". $row['kode_barang'] ."' data-barang='". $row['nama_barang'] ."' data-subtotal='". $row['subtotal'] ."'>Hapus</button>";
}
else{
