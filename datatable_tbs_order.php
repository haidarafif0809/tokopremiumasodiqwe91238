...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
  $nestedData=array(); 
      ...
      $nestedData[] = "<p style='font-size:15px' align='right' class='edit-jumlah' data-id='".$row['id']."'><span id='text-jumlah-".$row['id']."'>". koma($row['jumlah_barang'],2) ."</span> <input type='hidden' id='input-jumlah-".$row['id']."' value='".$row['jumlah_barang']."' class='input_jumlah' data-id='".$row['id']."' autofocus='' data-kode='".$row['kode_barang']."'  data-harga='".$row['harga']."' data-satuan='".$row['satuan']."' > </p>";
