...
$query_select_tbs = $db->query("SELECT no_faktur_order FROM tbs_pembelian WHERE no_faktur_order = '$no_faktur' ");
$data_select_tbs = mysqli_num_rows($query_select_tbs);

	if ($data_select_tbs > 0){
		$query_hapus_tbs = $db->query("DELETE FROM tbs_pembelian WHERE no_faktur_order = '$no_faktur'");
	}

$query_select_detail = $db->query("SELECT satuan, asal_satuan, no_faktur_order, tanggal, jam FROM detail_pembelian_order WHERE no_faktur_order = '$no_faktur'");
...
if ($data_select_detail['satuan'] == $data_select_detail['asal_satuan']) {
	...
	//INSERT DARI DETAIL pembelian ORDER KE TBS pembelian
		$insert_tbs_pembelian = "INSERT INTO tbs_pembelian (session_id, kode_barang, nama_barang, jumlah_barang, satuan, harga, subtotal, potongan, tax, no_faktur_order) SELECT '$session_id', kode_barang, nama_barang, jumlah_barang, satuan, harga, subtotal, potongan, tax, '$no_faktur' FROM detail_pembelian_order WHERE no_faktur_order = '$no_faktur' ";
...
$update_status_order = $db->query("UPDATE pembelian_order SET status_order = 'Di Proses' WHERE no_faktur_order = '$no_faktur' ");
