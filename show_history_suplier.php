...
$sql = "SELECT no_faktur, tanggal,total,jenis,no_faktur_terkait,pembayaran,saldo_hutang
FROM (SELECT no_faktur AS no_faktur, tanggal AS tanggal,total,CONCAT('Pembelian',' (',status_beli_awal,')') AS jenis,'' AS no_faktur_terkait,tunai AS pembayaran,nilai_kredit AS saldo_hutang
      ...
      FROM detail_pembayaran_hutang WHERE suplier = '$suplier' AND tanggal >= '$dari_tanggal' AND tanggal <= '$sampai_tanggal' ) AS p";
