...
$no_faktur = $nomor."/JL/".$data_bulan_terakhir."/".$tahun_terakhir;
    ...
    $query9 = $db->query("SELECT * FROM fee_produk WHERE nama_petugas = '$user' AND kode_produk = '$kode_barang'");
    ...
    if ($prosentase != 0){
      ...
      if ($cek01 > 0) {
        $query91 = $db->query("UPDATE tbs_fee_produk SET jumlah_fee = '$komisi' WHERE nama_petugas = '$user' AND kode_produk = '$kode_barang'");
      }
      ...
      else
      {
          ...
          $query10 = $db->query("INSERT INTO tbs_fee_produk (nama_petugas, no_faktur, kode_produk, nama_produk, jumlah_fee, tanggal, jam) VALUES ('$user', '$no_faktur', '$kode_barang',
...
// MENGAMBIL DATA STOK BARANG-->
        $perintah0 = $db->query("SELECT * FROM barang WHERE kategori = 'Makanan' OR kategori = 'Minuman'");
...
// MENGAMBIL DATA STOK BARANG--<
    ...
    else
    {
        ...
        if ($jumlah > 0)
            ...
            {
                $perintah02 = $db->prepare("INSERT INTO tbs_penjualan (session_id,kode_barang,nama_barang,jumlah_barang,satuan,harga,subtotal,hpp,no_pesanan) VALUES (?,?,?,?,?,?,?,?,'1')");
                
                
                $perintah02->bind_param("sssisiii",
                $session_id, $kode_barang, $nama_barang, $jumlah_barang, $satuan, $harga, $subtotal, $total_hpp);
