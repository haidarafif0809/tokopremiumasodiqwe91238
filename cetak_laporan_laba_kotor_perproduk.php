...
$sampai_tanggal = stringdoang($_GET['sampai_tanggal']);
  ...
  $penjualan = $db->query("SELECT kode_barang , nama_barang , SUM(jumlah_barang) AS jumlah_barang, SUM(subtotal) AS total_penjualan , SUM(potongan) AS total_potongan,(SELECT SUM(total_nilai) AS total_hpp FROM hpp_keluar WHERE kode_barang = detail_penjualan.kode_barang AND hpp_keluar.tanggal >= '$dari_tanggal' AND hpp_keluar.tanggal <= '$sampai_tanggal' ) AS total_hpp,SUM(subtotal) - (SELECT SUM(total_nilai) AS total_hpp FROM hpp_keluar WHERE kode_barang = detail_penjualan.kode_barang AND hpp_keluar.tanggal >= '$dari_tanggal' AND hpp_keluar.tanggal <= '$sampai_tanggal' ) AS total_laba FROM detail_penjualan WHERE tanggal >= '$dari_tanggal' AND tanggal <= '$sampai_tanggal' GROUP BY detail_penjualan.kode_barang ORDER BY SUM(subtotal) - total_hpp DESC");
