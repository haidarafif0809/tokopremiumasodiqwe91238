...
include 'persediaan.function.php';
    ...
    $no_faktur = stringdoang($_POST['no_faktur']);
        ...
        // UNTUK MENGETAHUI JUMLAAH TBS SEBENARNYA
            ...
            $query_stok_tbs = $db->query("SELECT jumlah_barang,satuan FROM tbs_pembelian WHERE kode_barang = '$kode_barang' AND no_faktur = '$no_faktur'");
            while($data_stok_tbs = mysqli_fetch_array($query_stok_tbs)){

                $jumlah_tbs_pembelian = $data_stok_tbs['jumlah_barang'];
                $jumlah_tbs = $jumlah_tbs_pembelian + $jumlah_tbs;
        ...
        // UNTUK MENGETAHUI JUMLAAH TBS SEBENARNYA
            ...
            // QUERY UNTUK CEK APAKAH SUDAH ADA APA BELUM DI TBS PENJUALAN
                $query_tbs_pembelian = $db->query("SELECT COUNT(kode_barang) AS jumlah_data FROM tbs_pembelian WHERE kode_barang = '$kode_barang' AND no_faktur = '$no_faktur' AND satuan = '$satuan'");
            ...
            // DISINI IF UNTUK CEK APAKAH BARCODE YANF DIMASUKAN ADA DIDAFTAR KODE BARANG
                ...
                if ($cek_row_barang <= 0){
                    echo 3;
                        ...
                        if ($data_tbs_pembelian['jumlah_data'] != 0) {// apablla barang ini sudah ada di tbs

                              $query1 = $db->prepare("UPDATE tbs_pembelian SET jumlah_barang = jumlah_barang + ?, subtotal = subtotal + ? WHERE kode_barang = ? AND no_faktur = ? AND satuan = ? ");
                              $query1->bind_param("iisss", $jumlah_barang, $a, $kode_barang, $no_faktur, $satuan);
                              $query1->execute();

                        }
                        else{

                              $perintah = $db->prepare("INSERT INTO tbs_pembelian (no_faktur,kode_barang,nama_barang,jumlah_barang,satuan,harga,subtotal) VALUES (?,?,?,?,?,?,?)");
                              $perintah->bind_param("sssisii", $no_faktur, $kode_barang, $nama_barang, $jumlah_barang, $satuan, $harga_tbs, $a);
