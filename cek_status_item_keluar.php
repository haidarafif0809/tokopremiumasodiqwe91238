...
$query_tbs_penjualan = $db->query("SELECT kode_barang,nama_barang,jumlah,satuan FROM tbs_item_keluar WHERE session_id = '$session_id' ");
...
while ($data_tbs_penjualan = mysqli_fetch_array($query_tbs_penjualan)) {
	...
	$stok = cekStokHpp($data_tbs_penjualan['kode_barang']);
		...
		$selisih_stok = $stok - $data_tbs_penjualan['jumlah'];
	...
	if ($selisih_stok < 0) {
		...
		"stok" => $stok,
		"jumlah_jual" => $data_tbs_penjualan['jumlah']
		);
