...
include 'persediaan.function.php';
    ...
    $kode_barcode = stringdoang($_POST['kode_barang']);
        ...
        // UNTUK MENGETAHUI JUMLAAH TBS SEBENARNYA
            ...
            $query_stok_tbs = $db->query("SELECT jumlah_barang,satuan FROM tbs_pembelian WHERE kode_barang = '$kode_barang' AND session_id = '$session_id'");
                ...
                if ($cek_row_barang <= 0){
                    echo 3;
                        ...
                        if ($data_tbs_pembelian['jumlah_data'] != 0) {// apablla barang ini sudah ada di tbs
                              ...
                              $query1 = $db->prepare("UPDATE tbs_pembelian SET jumlah_barang = jumlah_barang + ?, subtotal = subtotal + ? WHERE kode_barang = ? AND session_id = ? AND satuan = ? ");
                              ...
                              $perintah = $db->prepare("INSERT INTO tbs_pembelian (session_id,kode_barang,nama_barang,jumlah_barang,satuan,harga,subtotal) VALUES (?,?,?,?,?,?,?)");
