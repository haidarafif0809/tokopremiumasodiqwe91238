...
// JIKA AKUN TIDAK MUNCUL (AKUN AKUN YG SALAH INPUT) - START
if ($kategori == "lain-lain") {

    $jenis_akun = stringdoang($_POST['jenis_akun']);

    if ($jenis_akun == "daftar_akun") {
      // DAFTAR AKUN (1)

        $sql =" SELECT id, kode_daftar_akun, nama_daftar_akun, grup_akun, kategori_akun, tipe_akun ";
        $sql.=" FROM daftar_akun WHERE 1=1";
        ...
        //GRUP AKUN (1)
          ...
          $sql =" SELECT id, kode_daftar_akun, nama_daftar_akun, grup_akun, kategori_akun, tipe_akun ";
          $sql.=" FROM daftar_akun WHERE 1=1";
          $sql.=" AND (kode_daftar_akun LIKE '".$requestData['search']['value']."%'";  
          $sql.=" OR nama_daftar_akun LIKE '".$requestData['search']['value']."%' )";
        ...
        $sql.=" ORDER BY kode_daftar_akun ASC LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
      ...
      $data = array();
        ...
        while( $row = mysqli_fetch_array($query) ) {
          $nestedData = array();
            $nestedData[] = "<p>".$row["kode_daftar_akun"]."</p>";

            $nestedData[] = "<p class='padding-75 edit-nama' data-id='".$row['id']."'><span id='text-nama-". $row['id'] ."'>". $row['nama_daftar_akun'] ."</span><input type='hidden' id='input-nama-".$row['id']."' value='".$row['nama_daftar_akun']."' class='input_nama_akun' data-id='".$row['id']."' autofocus=''></p>";

          //TAMPIL KATEGORI AKUN (KOLOM KLASIFIKASI)
            $kategori_akun = "<p class='edit-kategori-akun' data-id='".$row['id']."'><span id='text-kategori-".$row['id']."'>". $row['kategori_akun'] ."</span>
                              <select style='display:none' id='select-kategori-".$row['id']."' value='".$row['kategori_akun']."' class='select-kategori-akun' data-id='".$row['id']."' autofocus=''>";
          ...
          //TAMPIL PARENT (KOLOM SUB AKUN)
            $parent = "<p Class='edit-parent' data-id='".$row['id']."'><span id='text-parent-".$row['id']."'>". $row['grup_akun'] ."</span>
                      <select style='display:none' id='select-parent-".$row['id']."' value='".$row['grup_akun']."' class='select-parent-akun' data-id='".$row['id']."' autofocus=''>";
                      $query_kode_grup = $db->query("SELECT kode_grup_akun FROM grup_akun");
                      while($data_kode_grup = mysqli_fetch_array($query_kode_grup)){
                        if ($data_kode_grup['kode_grup_akun'] == $row['grup_akun']) {
          ...
          //TAMPIL TIPE AKUN (KOLOM TIPE AKUN)
            $tipe_akun = "<p class='edit-tipe-akun' data-id='".$row['id']."'><span id='text-tipe-".$row['id']."'>". $row['tipe_akun'] ."</span>
                          <select style='display:none' id='select-tipe-".$row['id']."' value='".$row['tipe_akun']."' class='select-tipe-akun' data-id='".$row['id']."' autofocus=''>";
            $tipe_akun .= "
          ...
          // TOMBOL HAPUS 
            if ($daftar_akun['daftar_akun_hapus'] > 0) {   
              $nestedData[] = "<button class='btn btn-danger btn-floating btn-hapus-akun' data-id='". $row['id'] ."' data-akun='". $row['nama_daftar_akun'] ."' kode-akun='". $row['kode_daftar_akun'] ."'> <i class='fa fa-trash'> </i> Hapus </button>";
            }
            else{
              $nestedData[] = "";
            }
...
// JIKA AKUN TIDAK MUNCUL (AKUN AKUN YG SALAH INPUT) - PERTENGAHAN IF ELSE
else{
...
// JIKA AKUN TIDAK MUNCUL (AKUN AKUN YG SALAH INPUT) - END
