...
<h1> LAPORAN BUCKET SIZE</h1><hr>
<br>

<form id="perhari" class="form-inline"  method="POST" role="form">
          
<div class="form-group">
    <input type="text" class="form-control" id="kelipatan" autocomplete="off" name="kelipatan" placeholder="Kelipatan" onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);">
</div>
...
$(document).on('click','#btntgl',function(e) {
    ...
    var kelipatan = bersihPemisah(bersihPemisah(bersihPemisah(bersihPemisah($("#kelipatan").val()))));
    var daritgl = $("#daritgl").val();
    var sampaitgl = $("#sampaitgl").val();

    if (kelipatan == '') {
      alert("Kelipatan harus di isi");
      $("#kelipatan").focus();
    }
    else if(daritgl == ''){
    ...
    else
    {
        ...
        "emptyTable":     "My Custom Message On Empty Table"
          ...
          "ajax":{
            url :"proses_lap_bucket_size.php", // json datasource
             "data": function ( d ) {
                d.dari_tanggal = $("#daritgl").val();
                d.sampai_tanggal = $("#sampaitgl").val();
                d.kelipatan = $("#kelipatan").val()
