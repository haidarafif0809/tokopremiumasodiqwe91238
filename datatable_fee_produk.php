...
$sql.="FROM fee_produk f LEFT JOIN user u ON f.nama_petugas = u.id ";
$query=mysqli_query($conn, $sql) or die("datatable_fee_produk.php: get employees");
...
$sql = "SELECT f.id,f.nama_petugas,f.kode_produk,f.nama_produk,f.jumlah_prosentase,f.jumlah_uang,f.user_buat,u.nama ";
$sql.="FROM fee_produk f LEFT JOIN user u ON f.nama_petugas = u.id WHERE 1=1";
...
$query=mysqli_query($conn, $sql) or die("datatable_fee_produk.phpppp: get employees");
...
$fee_produk_edit = mysqli_num_rows($pilih_akses_fee_produk_edit);
    ...
    if ($fee_produk_edit > 0) {
			$nestedData[] = "<button class='btn btn-success btn-edit' data-prosentase='". $row['jumlah_prosentase'] ."' data-nominal='". $row['jumlah_uang'] ."' data-id='". $row['id'] ."' > <span class='glyphicon glyphicon-edit'> </span> Edit </button>";
		}
