...
$perintah3 = $db->query("SELECT no_faktur, kode_parcel FROM tbs_parcel WHERE no_faktur = '$no_faktur' AND kode_parcel = '$kode_parcel' ");
...
while ($data = mysqli_fetch_array($perintah)){
	...
	$subtotal = $data['harga_produk'] * $data['jumlah_produk'];

	$perintah1 = $db->query("INSERT INTO tbs_parcel (no_faktur, kode_parcel, id_produk, jumlah_produk, harga_produk, subtotal_produk) VALUES ( '$data[no_faktur]', '$data[kode_parcel]', '$data[id_produk]', '$data[jumlah_produk]', '$data[harga_produk]', '$subtotal')");
}
