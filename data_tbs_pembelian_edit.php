...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
  $nestedData=array(); 
      ...
      if ($otoritas_tombol['pembelian_edit'] > 0){
        ...
        $nestedData[] = "<p class='edit-jumlah' data-id='".$row['id']."' data-faktur='".$row['no_faktur']."' data-nama='".$row['nama_barang']."' data-kode='".$row['kode_barang']."'><span id='text-jumlah-".$row['id']."'>". $row['jumlah_barang'] ."</span> <input type='hidden' id='input-jumlah-".$row['id']."' value='".$row['jumlah_barang']."' data-nama='".$row['nama_barang']."' data-stok='".cekStokHpp($row['kode_barang'])."' data-over='".$data_produk['over_stok']."' class='input_jumlah' data-id='".$row['id']."' autofocus='' data-kode='".$row['kode_barang']."' data-harga='".$row['harga']."' > </p>";
