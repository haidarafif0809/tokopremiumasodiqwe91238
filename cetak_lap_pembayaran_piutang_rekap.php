...
$perintah = $db->query("SELECT p.nama_pelanggan,p.kode_pelanggan,pp.nama_suplier,pp.no_faktur_pembayaran,pp.tanggal,pp.dari_kas,pp.total, da.nama_daftar_akun FROM pembayaran_piutang pp INNER JOIN pelanggan p ON pp.nama_suplier = p.id INNER JOIN daftar_akun da ON pp.dari_kas = da.kode_daftar_akun WHERE pp.tanggal >= '$dari_tanggal' AND pp.tanggal <= '$sampai_tanggal'");
