...
function stokReturJual($kode_barang, $no_faktur)
{

  include 'db.php';

  $query_hpp_masuk = $db->query("SELECT SUM(jumlah_kuantitas) AS jumlah FROM hpp_keluar WHERE kode_barang = '$kode_barang' AND no_faktur = '$no_faktur'");

  $query_hpp_keluar = $db->query("SELECT SUM(jumlah_kuantitas) AS jumlah FROM hpp_masuk WHERE kode_barang = '$kode_barang' AND no_faktur_hpp_keluar = '$no_faktur'");
